...
- The validated live paths were `/dev/ttyACM0` for the F9P and `/dev/ttyUSB0` for the UM982.
...
- Prefer `/dev/serial/by-id` when it is available. If it is missing in the runtime, confirm the live device through `/sys/class/tty/*/../manufacturer` and `/sys/class/tty/*/../product` before wiring `GNSS_SERIAL_DEVICE`.
