...
void ObstacleTrackerNode::on_costmap(nav_msgs::msg::OccupancyGrid::ConstSharedPtr msg)
{
  ...
  // Only flood-fill cells the costmap reports as truly LETHAL (OG value 100,
  // internal cost 254). Lower thresholds picked up the inflation halo around
  // every lethal cell — including the polygon-edge keepouts. Those clusters
  // got promoted to PERSISTENT, fed back into obstacle_polygons_, and Re-
  // expanded the keepout mask inward by ~0.18 m every cycle. After a few
  // minutes the navigable area had shrunk far enough that opennav_docking's
  // NavigateToPose to the staging pose ((-1.5, 0) in our sim) returned
  // error 208 "no valid path found" from inside-polygon starts.
  // Inflation around tracked obstacles is regenerated by the costmap's own
  // inflation_layer once we re-publish the cluster polygon as keepout, so
  // we don't lose the safety margin — we just stop double-counting it.
  constexpr int8_t OBSTACLE_COST = 100;
  std::vector<bool> visited(w * h, false);
  std::vector<std::vector<std::pair<double, double>>> clusters;

  // Lambda: cell-position polygon membership test. Returns true if the
  // boundary is loaded AND the cell is OUTSIDE the polygon (i.e., should
  // be skipped). Without this filter, the keepout-mask LETHAL frame
  // surrounding the polygon flood-fills as one huge cluster, gets merged
  // with every interior obstacle by merge_overlapping, and ends up as a
  // single tracked-obstacle polygon spanning the entire mowing area.
  // That polygon then lands in obstacle_polygons_ → keepout_mask, marking
  // most of the polygon LETHAL and breaking opennav_docking's NavigateTo
  // -Pose to the staging pose with "Start occupied" / "no valid path".
  const auto cell_outside_polygon = [&](int cx, int cy) -> bool
  {
    if (!boundary_loaded_)
      return false;
    const double mx = ox + (cx + 0.5) * res;
    const double my = oy + (cy + 0.5) * res;
    return !point_in_polygon(mx, my, boundary_polygon_);
  };
  ...
  for (int y = 0; y < h; ++y)
  {
    for (int x = 0; x < w; ++x)
    {
      ...
      if (visited[idx] || msg->data[idx] < OBSTACLE_COST)
        continue;
      if (cell_outside_polygon(x, y))
      {
        visited[idx] = true;  // mark visited so we never seed from here
        continue;
      }
