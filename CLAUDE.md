...
8. **FTCController for coverage paths; RPP (via RotationShim) for transit** — the `FollowCoveragePath` slot uses `mowgli_nav2_plugins/FTCController` (required for the <10 mm lateral accuracy that makes swath-tiling actually tile). The `FollowPath` (transit) slot uses `nav2_rotation_shim_controller` wrapping `nav2_regulated_pure_pursuit_controller` (RPP) — see `nav2_params.yaml`; `mowgli_bringup/package.xml` depends on `nav2_regulated_pure_pursuit_controller`. (Historical note: an earlier plan moved BOTH slots to FTC and these docs said "RPP is gone" — that is **not** the current config; RPP is live for transit.) MPPI was rejected for coverage because its Euclidean nearest-point matching jumps between adjacent swaths.
