...
8. **Controllers: FTCController for BOTH transit (`FollowPath`) and coverage (`FollowCoveragePath`), with separate gains per slot.** *Changed on `feat/v2-control-cascade` (2026-06-08): `FollowPath` was `RotationShimController`+`RegulatedPurePursuitController` (RPP), now `mowgli_nav2_plugins/FTCController`.* This deliberately mirrors the ROS1 "v2" stack, which used Follow-The-Carrot for transit + coverage + docking and rotated/docked reliably. RPP was removed from `FollowPath` because it could **not** fine-pivot on this robot's static-friction-deadband motors: `use_rotate_to_heading` had to be disabled (RPP pivot-locked and hunted), so RPP only arced and left a yaw-alignment gap at goals — most damaging at the `opennav_docking` staging pose (arriving 10-30° off the dock axis stalled the approach). FTC's smooth rate-limited carrot + PRE_ROTATE/POST_ROTATE align heading in place. The two slots keep **independent** gains: `FollowPath` uses the more aggressive v2 transit rotation gains (`kp_ang 3.0`, `speed_angular 45`), `FollowCoveragePath` stays detuned for swath-tiling (`kp_lat 1.2`, `kp_ang 1.5`, <10 mm lateral accuracy). FTC's internal `max_goal_angle_error`/`max_goal_distance_error` on `FollowPath` MUST stay ≤ the `stopped_goal_checker` tolerances or the action hangs (FTC stops at its own tolerance while the checker waits) — see `test_followpath_goal_tolerance_tighter_than_checker`. MPPI is still rejected for coverage (Euclidean nearest-point matching jumps between adjacent swaths). *History: RPP-for-transit was the prior steady state (upstream `9ef5d2f7` switched it from MPPI); the gain that motivated keeping FTC-for-transit out was uncertainty about lateral whipsaw, now mitigated by per-slot gains + open-loop firmware drive (`USE_WHEEL_PI=0`). Still under field validation — the gyro angular-rate loop (`angular_rate_loop_enabled`, in `hardware_bridge`) is the key A/B for whether the cascade needs it.*
...
- **Navigation:** `FollowPath` (transit / return / dock-approach) = `mowgli_nav2_plugins/FTCController` (Follow-the-Carrot) since `feat/v2-control-cascade` — PRE_ROTATE aligns to the path-start heading in place, FOLLOWING arcs the (Hybrid-A*) path, POST_ROTATE aligns to the goal heading; transit-tuned gains (`kp_ang 3.0`, `speed_angular 45`) distinct from coverage. `FollowCoveragePath` (coverage strips) = `FTCController` with coverage gains (3-axis PID; <10 mm lateral accuracy needed for swath-tiling). NOT MPPI for coverage (jumps between adjacent swaths). *Was `RotationShimController`+RPP for transit until 2026-06-08; removed because RPP couldn't fine-pivot the deadband-limited motors (see invariant #8).*
