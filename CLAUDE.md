...
Open-source autonomous robot mower monorepo. ROS2 Kilted, Nav2, and a GTSAM iSAM2 factor-graph localizer (`fusion_graph`, in `ros2/src/fusion_graph/`) that is now the **default and sole** map+odom localizer (`use_fusion_graph: true` in `mowgli_robot.yaml`), owning **both** `map→odom` and `odom→base_footprint`, with optional LiDAR scan-matching + loop-closure factors. BehaviorTree.CPP v4, cell-based strip coverage. **The robot_localization dual-EKF (`ekf_map_node` + `ekf_odom_node`) and `config/robot_localization.yaml` were removed** — `navigation.launch.py` is the source of truth for the localizer wiring. Some sections below still describe the old dual-EKF as "default" and fusion_graph as "opt-in"; those are **historical** — read them as how it *used* to be wired. (`docs/HANDOFF_FUSION_GRAPH.md` is also historical.)
...
1. **Localizer = `fusion_graph` (GTSAM), sole and default.** `use_fusion_graph: true` in `mowgli_robot.yaml`; `fusion_graph_node` publishes **both** `map→odom` AND `odom→base_footprint` (replacing the removed `ekf_map_node` and `ekf_odom_node`). `config/robot_localization.yaml` no longer exists. **The bullets immediately below are HISTORICAL** (the pre-removal dual-EKF design) — kept for context on the inputs/covariance conventions, which fusion_graph still consumes, but the dual-EKF nodes themselves are gone. Historical design:
...
- **Drive base = ros2_control (`mowgli_hardware`).** The motor path is the standard chained stack: `diff_drive_controller` → per-wheel `pid_controller` ×2 → **`MowgliSystemInterface`** (a `hardware_interface::SystemInterface` plugin, `ros2/src/mowgli_hardware/src/mowgli_system_interface.cpp`). The plugin OWNS the STM32 COBS/CRC serial link and EMBEDS the `HardwareBridgeNode` comms core (`hardware_bridge_node.cpp`, node name `hardware_bridge`) which still does ALL non-motor I/O verbatim — IMU+bias-cal, status/power/battery, blade, emergency+heartbeat, HL-state mirror, charging, dock-heading, `/wheel_odom` (force-zeroed on charging, non-holo `vy`=1e-4 covariance), `/wheel_ticks`, reboot. The plugin only converts the commanded wheel velocity → raw PWM via a static feedforward (`velocity_pwm.hpp`: slope `pwm_per_mps` + break-free `deadband_pwm`) and relays it; the `pid_controller` closes the per-wheel velocity loop. `<ros2_control>` block is in `mowgli.urdf.xacro` (gated on `use_ros2_control`); controllers in `config/mowgli_controllers.yaml`; spawned by `mowgli.launch.py` (controller_manager + spawners, NO standalone `hardware_bridge_node` executable — the controller_manager process owns the serial link). **`diff_drive_controller.enable_odom_tf` MUST stay false** (fusion_graph owns `odom→base_footprint`); its `wheel_radius`/`wheel_separation` are injected from the SAME `mowgli_robot.yaml` values the plugin uses (the radian cancels only if they agree; `ticks_per_meter` is the true ground scale). **This REPLACED the hand-rolled host wheel-velocity PI (`wheel_rate_controller.hpp`) + the gyro angular-rate loop (`angular_rate_controller.hpp`) — both deleted.** The gyro loop is gone on purpose: the per-wheel velocity loop absorbs the nonlinear PWM curve, residual slip is the localizer's job. Speed-tracking (EMA-de-quantised joint velocity) is validated; pivot-accuracy without the gyro loop still needs an on-robot motion test (deferred). `tools`: gains in `mowgli_controllers.yaml` are conservative starting points pending on-robot tuning.
...
- Do NOT publish a `map→odom` OR `odom→base_footprint` TF from any node other than `fusion_graph_node` — it is the sole localizer and owns both (the `ekf_map_node`/`ekf_odom_node` it replaced are gone). In `fusion_graph`'s observer mode (`primary_mode:=false`, sim/A-B only) it still must not broadcast TF.
