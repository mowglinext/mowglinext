...
8. **Two controllers: RotationShim+RPP for transit (`FollowPath`), FTCController for coverage (`FollowCoveragePath`).** `FollowPath` uses `nav2_rotation_shim_controller::RotationShimController` wrapping `RegulatedPurePursuitController` (RPP) — a deliberate choice (upstream `9ef5d2f7` switched it from MPPI, actively tuned since): RotationShim pivots in place to face the next path segment, then hands off to RPP. `FollowCoveragePath` uses `mowgli_nav2_plugins/FTCController` (coverage churned FTC→MPPI→back-to-FTC; FTC's <10 mm lateral accuracy is required for swath-tiling). MPPI was rejected for coverage because its Euclidean nearest-point matching jumps between adjacent swaths. Do NOT "unify on FTCController for both" — FTC-for-`FollowPath` is **not** the architecture (an earlier version of this file wrongly claimed it).
...
- **Navigation:** `FollowPath` (transit / return / dock-approach) = `RotationShimController` + `RegulatedPurePursuitController` (RPP) primary — RotationShim pivots in place to the next segment heading, then RPP follows. `FollowCoveragePath` (coverage strips) = `FTCController` (Follow-the-Carrot with 3-axis PID; <10 mm lateral accuracy needed for swath-tiling). NOT MPPI for coverage (jumps between adjacent swaths). RPP runs ONLY on the transit slot, never on coverage.
