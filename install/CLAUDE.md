...
- `compose/docker-compose.foxglove.yml` is dead and would break the stack (`enable_coverage:=`, an arg no launch file declares). In MAVROS mode, NTRIP belongs inside the pinned external sidecar; do not add a standalone MowgliNext NTRIP service because `mowgli_ntrip_client` is absent from `ros2/src`.
