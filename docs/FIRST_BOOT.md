...
- Fastest path: GUI → **Settings → Docking** → **Start dock calibration** (one-click). With the robot on the dock, charging, and RTK-Fixed, it reverses in a straight line, fits the dock heading from the GPS course and saves that **heading** right there (off the dock), then re-docks and — once charging is confirmed — saves the dock **position**. If the re-dock fails the heading stays saved but the position is not updated. It needs a dock position to already be stored (it refuses to pair a fresh heading with a `0, 0` position): on a brand-new install press **Set dock pose** once first, robot on the dock. The blade stays off throughout.
