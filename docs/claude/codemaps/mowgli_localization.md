...
| One-click dock calibration (CalibrateDock action / GUI start service / status topic) | `ros2/src/mowgli_localization/src/calibrate_imu_yaw_node.cpp` `run_dock_calibration_core()`, `persist_dock_via_map_server()`, `dock_start_cb()` |
| Pre-reverse dock-POSITION capture (mowglinext#446) — averaged, on-dock, before reversing | `calibrate_imu_yaw_node.cpp` `raw_gps_fix_cb()` (RTK-Fixed `/gps/fix` → raw antenna ENU, mirrors `map_server_node`'s `recent_gps_antenna_enu_`), `try_average_dock_position()`, `wait_for_dock_position()` — called right before the reverse leg in `run_dock_calibration_core()`; aborts the whole run (`RETRY_NO_RTK`/`RETRY_WRONG_STATE`) if it can't be established, rather than reversing for nothing |
...
| `/map_server_node/set_docking_point` | `mowgli_interfaces/srv/SetDockingPoint` | **client** (L310): `use_gps_position=false` (explicit `docking_pose.position`, captured pre-reverse — see `wait_for_dock_position()`), `yaw_source=MOTION` (mowglinext#446) | one-click persist path, `persist_dock_via_map_server()` |
...
- **Three dock-pose file writers were collapsed to `robot_yaml_scalar::UpdateDockPose`, but two callers remain**: `map_server` (`area_manager.cpp`, the canonical `/set_docking_point` path) and the legacy `~/calibrate` dock pre-phase (`calibrate_imu_yaw_node.cpp`). `mowgli_behavior/calibration_nodes.cpp` no longer writes. The one-click action routes through map_server (`persist_dock_via_map_server()`) as soon as the reverse leg's COG-coherence gate passes — NOT after the subsequent re-dock, which is only a live confirmation pass (`docking_server`/`gps_dock_detection_node` read `dock_pose_x/y/yaw` as ROS params at container startup only, so a same-session re-dock can't benefit from a deferred write anyway; see the persist block's comment in `calibrate_imu_yaw_node.cpp`).
- **The position this persist sends is NOT map_server's own live GPS average** (mowglinext#446, fixing a real regression from the reorder above): `persist_dock_via_map_server()` sends `use_gps_position=false` with an explicit position, captured and frozen by `wait_for_dock_position()`/`try_average_dock_position()` BEFORE the reverse leg starts. Sending `use_gps_position=true` here (the original, pre-#446 code) would average `map_server_node`'s continuously-running `recent_gps_antenna_enu_` window at PERSIST time — by then contaminated by the reverse leg's own motion — and would also, unconditionally, fail `on_set_docking_point`'s `is_charging` gate, since the robot has deliberately left the dock by that point. `on_set_docking_point` exempts exactly the `yaw_source=MOTION && !use_gps_position` combination from that gate (see `mowgli_map` codemap) because the position was already captured under verified on-dock conditions earlier in the same run.
