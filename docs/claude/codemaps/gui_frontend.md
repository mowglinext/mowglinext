...
Settings updates: `components/settings/HostUpdaterPanel.tsx` and `hooks/useHostUpdater.ts` implement Simple/matched updates, Advanced compatible versions for every managed service, collapsed preferences/history/diagnostics, cached runtime identity/health and source selection. Browser freshness compares a build ID embedded in JavaScript with `web-build.json` generated by Vite, separately from the Go backend revision (`hooks/useInstalledVersions.ts`). Fixture screenshots and behavior coverage: `tests/e2e/host-updater.spec.ts`.
