...
| LiDAR localizer toggles (settings) | `components/settings/LocalizationSection.tsx` `LIDAR_FACTOR_TOGGLES`: `use_scan_matching`, `use_loop_closure`, `use_lidar_map_anchor`, `lidar_anchor_shadow_mode` (the shadow toggle is `dependsOn: "use_lidar_map_anchor"` and is disabled while the anchor is off). Keys listed in `hooks/useSettingsManager.ts` `localization` section + `paramCatalog.ts`; they write straight through `POST /settings/yaml` (no schema entry) |
