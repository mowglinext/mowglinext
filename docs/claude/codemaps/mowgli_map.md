...
| Dock pose set gates (charging / RTK σ / yaw convergence / GPS averaging / yaw_source) — and WHICH request kind needs which gate | `include/mowgli_map/dock_set_gates.hpp` `ResolveDockSetGates` (pure, `test_dock_set_gates`); `area_manager.cpp` `on_set_docking_point`; write-back via `mowgli_interfaces/robot_yaml_scalar.hpp` `UpdateDockPose` (L122) |
...
| `~/set_docking_point` | `mowgli_interfaces/srv/SetDockingPoint` (`use_gps_position`, `preserve_position`, `yaw_source` PRESERVE/REQUEST/MOTION, `yaw_rad` → `success`, `message`) | `mowgli_localization/src/calibrate_imu_yaw_node.cpp` L310; GUI `mowglinext.go` L238 |
...
| `keepout_obstacle_margin` | -1.0 = derive `chassis_width/2`; clamped [0,1] | `full_system.launch.py` (`robot_config_util.keepout_obstacle_margin`, ≈0.276 shipped) | lethal band around drawn / dig / promoted obstacle polygons = the body half-width, counted ONCE (Smac 2D is a point check; the mask is not inflated). NOT `coverage_server.obstacle_margin`; also sets the dig keepout's rear-edge offset |
...
- **ONE request kind is exempt from the charging + fused-yaw-convergence gates: `preserve_position=true` + `yaw_source=MOTION` + `use_gps_position=false`** (`dock_set_gates.hpp`). It is the dock calibration's yaw-only write: the yaw is measured by reversing OFF the dock, so the robot is ~1.5 m from the charger and the charging gate can never pass (a position capture sent from there failed every run, 2026-09-17). It keeps the RTK-accuracy gate, requires a dock position to already be stored, and leaves X/Y untouched server-side. Do NOT widen it: anything that captures or sets a POSITION keeps every gate. Any other `preserve_position` combination is rejected as invalid.
