...
> Index updated 2026-09-09 for FTC obstacle-hold recovery; regenerate when files are added/removed.
...
| Smooth recovery after obstacle hold (continuous-clear debounce, PID/carrot reset, speed ramp) | `include/mowgli_nav2_plugins/ftc_obstacle_wait.hpp`, `ftc_stall.hpp` `ClampForwardToMovementRamp()`, and `ftc_controller.cpp` `holdObstacleMotion()` |
...
| `ftc_controller.hpp` | 583 | `FTCController` class: FSM enum, carrot/PID/deviation/reverse/oscillation state, `struct Config` (all params + C++ defaults) |
| `ftc_stall.hpp` | 95 | Pure `StallDecision()` plus acceleration-ramp clamp used after obstacle holds |
| `ftc_obstacle_wait.hpp` | 31 | Pure continuous-followable debounce for obstacle-hold recovery |
...
| `test_ftc_obstacle_wait.cpp` | 41 | 3 cases: continuous-clear hold, blocked-scan reset, alternating scans never resume |
...
- `test/test_ftc_obstacle_wait.cpp` — an obstacle wait resumes only after a continuous followable window; any blocked scan resets the evidence.
