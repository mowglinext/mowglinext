...
| `sensors-openmower.yml` | push (same branch set), paths `sensors/openmower/**`, `ros2/src/mowgli_interfaces/**`, `ros2/src/mowgli_hardware/**`, self, `_sensor-docker.yml` | via `_sensor-docker.yml` + own `test` job | Builds the `openmower` image (smoke: package, executable, launch + config installed, no dangling `ldd`); the `test` job assembles `mowgli_interfaces` + `mowgli_hardware` (tests off) + `mowgli_openmower_bridge` and runs its 47 gtests, asserting a non-zero case count | — | The ONLY place `mowgli_openmower_bridge`'s tests run. |
