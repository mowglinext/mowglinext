...
1. **Branch name matters for push-triggered workflows.** Use `feat/`, `fix/`, `refactor/`, `chore/` or `perf/`. The message and protocol drift workflows run for PRs targeting `main` or `dev`, subject to their existing file-path filters.
